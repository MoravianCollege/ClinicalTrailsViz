...
studies_gender <- dbGetQuery(con, "SELECT * FROM studies NATURAL JOIN eligibilities WHERE overall_status='Completed'")
...
plot <- ggplot(data=x, aes(x=gender, y=count)) + geom_bar(stat="identity")+ 
  labs(title = "Total Number of Completed Trials by Gender",
       caption = "The histogram above visulaizes the number of completed trials per the gender of eligibility") + 
